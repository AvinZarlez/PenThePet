...
def solve_map(map_data, max_walls):
    ...
    Args:
        map_data: 2D list of tile type strings ("grass", "water", "home")
    ...
    # Variables: w[i] = 1 if wall placed on grass tile i
    w = {}
    ...
    prob += lpSum(w[tile] for tile in grass_tiles) <= max_walls

    # Constraint 5: vertex cut - adjacent tiles on different sides need a wall
    # For each edge (i,j): s[i] - s[j] <= w_i + w_j
    for (i, j) in edges:
        w_i = w[i] if i in grass_set else 0  # home has no wall variable
        w_j = w[j] if j in grass_set else 0
