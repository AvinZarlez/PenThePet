...
def load_tile_scores():
    """
    Load tile score values from js/tileData.js via a generated JSON sidecar.
    Falls back to reading tileData.js directly and extracting score values.

    Returns:
        dict mapping tile name (str) → score (int)
    """
    # Try loading the JSON sidecar first (generated by build step)
    sidecar = os.path.join(os.path.dirname(__file__), 'tile_scores.json')
    if os.path.exists(sidecar):
        with open(sidecar, 'r') as fh:
            return json.load(fh)

    # Fallback: parse tileData.js to extract score values
