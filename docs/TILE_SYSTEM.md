...
The tile system is the foundation of PenThePet's game mechanics. All tile types are defined in a single data file (`js/tileData.js`) that serves as the **single source of truth** for game logic, rendering, map generation, solving, and player-facing instructions.

## Adding a New Tile Type

To add a new tile, add a single entry to `TILE_DATA` in `js/tileData.js`. No other code changes are needed — all derived lookup tables, rendering, generation, pathfinding, scoring, and player instructions are built automatically from this data.
...
Then add a CSS rule for cursor/hover behavior and the SVG asset.
...
| `ariaLabel` | function | ✅ | `(row, col) => string` for screen reader labels |
...
tileData.js (TILE_DATA)
    ...
    └── solve.py        — reads scores (load_tile_scores) and properties (load_tile_properties)
```
